...
export async function connect(): Promise<boolean>{
    try{
        ...
        let command = os.userInfo().username!='root' ? `sudo wg-quick up ${path}` : `wg-quick up ${path}`
...
export async function disconnect(): Promise<boolean> {
    try{
            const homeDir = os.homedir();
            const path = `${homeDir}/${foldername}/${wginterface}.conf`
            let command = os.userInfo().username!='root' ? `sudo wg-quick down ${path}` : `wg-quick down ${path}`
            const childProcess = await spawn(command, { shell: true });
            return new Promise<boolean>((resolve) => {
                childProcess.on('close', (code: number) => {
                  if (code === 0) {
